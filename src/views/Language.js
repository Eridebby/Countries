 ...
 useEffect(() => {
    async function findData() {
      try {
        ...
        const res = await fetch(
          `https://restcountries.com/v3.1/lang/${query}`
        );
  ...
  return (
    <div>
      ...
      {!dataError && !loaderQuery && <Lang lang={dataQuery} />}
      {dataError && 
        <div
          style={{
            margin: "auto",
  ...
  return (
    ...
    {lang.map((item, index) => {
      return (
        ...
        <p className="card-text">{item.car.side === "right" ? (
               <AiOutlineArrowRight />
            ) : (
              <AiOutlineArrowLeft />
           )}</p>
           <img src={item.coatOfArms.png} className="card" style={{ width: "30px" }} />
